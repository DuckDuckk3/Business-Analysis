...
| :--- | :--- | :--- |
| **01** | **Business Entity** | The business is legally and financially separate from its owners. Crucial for data isolation and multi-tenancy. |
| **02** | **Money Measurement** | Only transactions quantifiable in a monetary unit are recorded. Non-monetary events (e.g., employee morale) are excluded. |
| **03** | **Going Concern** | The assumption that the company will operate indefinitely, justifying long-term asset depreciation over immediate expensing. |
| **04** | **Accounting Period** | The life of a business is divided into artificial time intervals (months, quarters, years) for periodic reporting. |

---

## B. Operational Principles (The Playing Rules)
These rules dictate how transactions are measured, recorded, and reported to ensure consistency and reliability.

### 1. The Dual Aspect (The Binary Logic)
Every transaction affects at least two accounts. This is the origin of the **Fundamental Accounting Equation**:
...
*   **Matching Principle:** Every dollar of revenue must be matched with the expenses incurred to generate it within the same period. This is the "Holy Grail" of calculating true profit.
...
## C. Qualitative Constraints (The Guardrails)
...
## D. The Accounting Cycle (The Data Pipeline)
The systematic process (ETL - Extract, Transform, Load) of turning raw business transactions into audited financial reports.

1.  **Transaction Analysis:** Identifying economic events from source documents.
2.  **Journalizing:** Recording transactions chronologically in the General Journal.
3.  **Posting:** Transferring journal entries to the General Ledger (categorizing by account).
4.  **Trial Balance:** Verifying that total Debits equal total Credits.
5.  **Adjusting Entries:** Applying Accrual/Matching principles at the end of the period.
6.  **Financial Statements:** Generating the final reports for stakeholders.
7.  **Closing Entries:** Resetting **Temporary Accounts** (Revenue/Expenses) to zero for the next period.
...
| **Income Statement** | Period (Video) | Measures profitability (Revenue - Expenses) over a specific duration. |
| **Cash Flow Statement**| Period (Video) | Reconciles net income to actual cash generated/used in operations, investing, and financing. |
| **Retained Earnings** | Period (Video) | Tracks how much profit is kept in the business vs. paid out to owners. |
